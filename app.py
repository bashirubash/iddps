...
def create_app():
    ...
    def transfer():
        ...
        if not from_acct or not to_acct:
            return jsonify({"error": "Invalid account number(s)"}), 404
        ...
        features = guard.build_features(
            system_load=0.6,
            wait_depth=WaitEdge.estimated_wait_depth(tx.id),
            amount=amount,
            hotspot=1 if from_acct.balance < 1000 else 0
        )
        risk = guard.predict_risk(features)
        mitigation = "reorder" if risk >= 0.8 else None

        # Locking order
        order = sorted([from_acct.id, to_acct.id])
        if mitigation == "reorder":
        ...
        if tx.status == "aborted":
            return jsonify({"status": "deadlock_aborted", "tx": tx.id}), 409
        ...
        if from_acct.balance < amount:
            lm.release_all(tx.id)
            tx.status = "aborted"
            tx.meta = {"reason": "insufficient_funds"}
            db.session.commit()
            return jsonify({"status": "insufficient_funds"}), 422
        ...
        return jsonify({"status": "ok", "tx": tx.id}), 200
